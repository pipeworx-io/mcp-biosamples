...
`tools/list` at `https://gateway.pipeworx.io/biosamples/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_samples \
  -H 'Content-Type: application/json' \
  -d '{"query":"liver"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_samples`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "biosamples": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-biosamples"]
    }
  }
}
```
